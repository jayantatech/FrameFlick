...
import ResponsiveTitle from "../responsiveTitle/responsiveTitle"; // Remove this line

const MovieHeroInfo = ({ testData, storeGen }) => {
  const averageVote = testData && testData?.vote_average;
  const votePointer = Number(
    testData && testData?.vote_average.toString().split(".")[1]
  );
        ...
        <div className=" w-full min-h-[230px] max-lg:min-h-[270px] max-md:min-h-[470px] rounded-lg  bg-[#636262] backdrop-filter backdrop-blur-sm bg-opacity-60 p-4 max-lg:px-2 flex flex-row gap-2 max-md:flex-col">
          <div className=" md:w-3/6 ">
            <h2 className=" text-3xl font-[Roboto] font-bold text-white min-h-[40px]">
              {testData?.original_title}
            </h2>
            <ResponsiveTitle data={testData} />{" "}
            {/* Use the imported ResponsiveTitle */}
              ...
              {testData && (
                <div className="flex items-center justify-center gap-2 bg-white px-2 h-full rounded-md">
                  ...
                  <div className="flex items-center justify-center gap-1 text-xl max-lg:text-lg font-bold">
                    {testData && timeFormath(testData?.release_date)}
                  </div>
                </div>
              )}
            </div>
            {testData && (
              <div className=" w-full h-[60px] bg-fuchsia-400  flex items-center justify-start px-2 flex-row gap-2">
                {/* {storeGen.map((item) => (
                  <span
                    key={item.id}
