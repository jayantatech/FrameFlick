...
const HeroMovie = () => {
  ...
  const [storeCast, setStoreCast] = useState([]);

  const testData = data?.results[6];

  useEffect(() => {
    const genRanNum = Math.floor(Math.random() * data?.results?.length);
    setFilmInfo(data?.results[genRanNum]);
  }, []);
  ...
  const { data: castData, loading: castLoading } = fetchData(
    `movie/${1139829}/credits?language=en-US'`
  );

  useEffect(() => {
    if (!castLoading && castData) {
      setStoreCast(castData?.crew?.slice(0, 10));
    }
  }, [castData]);
  return (
    <div className=" w-full h-[590px] max-md:h-[670px] bg-red-400 relative">
      ...
      <MovieHeroInfo filmInfo={filmInfo} storeGen={storeGen} />
    </div>
  );
};
